...
bool OxtsDriver::checkRate(double prevPktSec, double currPktSec)
{
  ...
  if (prevPktSec <= 0)
    ...
    RCLCPP_ERROR(
      this->get_logger(), "Current packet is older than previous packet, skipping packet.");
    skip_packet = true;
  } else if (currPktSec == prevPktSec) {
...
rclcpp::Time OxtsDriver::getNcomTime(const NComRxC * nrx)
{
  auto time = rclcpp::Time(
    static_cast<int32_t>(nrx->mTimeWeekSecond) + (nrx->mTimeWeekCount * NAV_CONST::WEEK_SECS) +
